function soundSegment=SegmentInfo(express)
if express(1)==0
    voicedIndex=find(express);                     % 若第一个SpeechIndex为0，寻找express中为1的位置
else
    voicedIndex=express;
end
...
for i=1:length(voicedIndex)-1,
	if voicedIndex(i+1)-voicedIndex(i)>1,          % 若Index之间有跳跃，语音结束
